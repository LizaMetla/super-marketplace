...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    customer_money_lost NUMERIC(15, 2) NOT NULL, -- сумма, которую потратил заказчик
...
COMMENT ON TABLE dwh.customer_report_datamart IS 'витрина заказчиков';
...
COMMENT ON COLUMN dwh.customer_report_datamart.customer_money_lost IS 'сумма, которую потратил заказчик';
...
dwh_update_delta AS ( -- делаем выборку по заказчикам, по которым были изменения в DWH. По этим заказчикам данные в витрине нужно будет обновить
    SELECT
...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. Этой информации по мастерам в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
    SELECT
            ...
            T5.customer_money_lost AS customer_money_lost,
                        ...
                        *
                        FROM (
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                                ...
                                SUM(T1.product_price) AS customer_money_lost,
                                ...
                                INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самого популярного мастера ручной работы у клиента. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                            ...
                                            ROW_NUMBER() over(partition by customer_id order by count(dd.craftsman_id) desc) as row_number
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id
                                                    ORDER BY count_craftsman_id DESC) AS T3 ON T2.customer_id = T3.customer_id_for_top_craftsman_id and T3.row_number = 1

                        INNER JOIN (
                                    SELECT     -- Эта выборка поможет определить самый популярный товар у клиента. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                            dd.customer_id AS customer_id_for_top_product_type,
                                            dd.product_type,
                                            COUNT(dd.product_type) AS count_product,
                                            ROW_NUMBER() over(partition by customer_id order by count(dd.product_type) desc) as row_number
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.product_type
                                                    ORDER BY count_product DESC) AS T4 ON T2.customer_id = T4.customer_id_for_top_product_type and T4.row_number = 1
                ) AS T5 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров
),
...
update_delta AS ( -- выполняем обновление показателей в отчёте по уже существующим мастерам
    UPDATE dwh.customer_report_datamart SET
        ...
        customer_money_lost = updates.customer_money_lost,
